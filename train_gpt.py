...
try:
    ...
    try:
        ...
        _FA3_AVAILABLE = False
        def flash_attn_3_func(q, k, v, causal=False, **kwargs):
            # Fallback to PyTorch SDPA (works on all GPUs)
            q = q.transpose(1, 2)
            k = k.transpose(1, 2)
            v = v.transpose(1, 2)
...
class Hyperparameters:
    ...
    # Depth recurrence: tile num_layers unique blocks recurrence_factor times
    # effective_layers = num_layers * recurrence_factor
    # Set to 1 to disable (standard mode). Set to 2 for 2x depth at same param cost.
    recurrence_factor = int(os.environ.get("RECURRENCE_FACTOR", 1))
...
CONTROL_TENSOR_NAME_PATTERNS = tuple(
    ...
    for pattern in os.environ.get(
        ...
        "attn_scale,attn_scales,mlp_scale,mlp_scales,resid_mix,resid_mixes,q_gain,skip_weight,skip_weights,smear,dtg_gate,ve_layer_scales,ve_shared.scale,pass_scales",
    ).split(",")
    if pattern
)
...
class GPT(nn.Module):
    def __init__(
        ...
        # Depth recurrence: num_layers unique blocks, tiled recurrence_factor times
        ...
        # Only allocate num_layers unique blocks (not effective_layers)
        ...
        # Per-pass learnable scales for recurrence (allows model to control each pass)
        if recurrence_factor > 1:
            self.pass_scales = nn.Parameter(
                torch.full((effective_layers,), pass_scale_init, dtype=torch.float32)
            )
        else:
        ...
        # VE layer indices refer to effective layer positions
    ...
    def _block_for_eff(self, eff_i: int) -> nn.Module:
        """Map effective layer index to unique block via modulo."""
        return self.blocks[eff_i % self.num_unique_layers]
    def _apply_block(self, eff_i: int, x: Tensor, x0: Tensor, v_embed: Tensor | None = None) -> Tensor:
        """Run a block with optional recurrence scaling."""
        out = self._block_for_eff(eff_i)(x, x0, v_embed=v_embed)
    ...
    def forward(self, input_ids: Tensor, target_ids: Tensor) -> Tensor:
        ...
        for i in range(self.num_encoder_layers):
            ...
            x = self._apply_block(i, x, x0, v_embed=ve)
            skips.append(x)
        for i in range(self.num_decoder_layers):
            ...
            x = self._apply_block(bi, x, x0, v_embed=ve)
        x = self.final_norm(x)
...
def main() -> None:
    ...
    if _FA3_AVAILABLE:
        # Hopper GPU with FA3: disable all SDPA backends (FA3 called directly)
        enable_cudnn_sdp(False)
        enable_flash_sdp(True)
        enable_mem_efficient_sdp(False)
        enable_math_sdp(False)
    else:
        # Non-Hopper GPU: enable all SDPA backends as fallback
    ...
    log0(f"depth_recurrence: unique={base_model.num_unique_layers} effective={base_model.effective_layers} factor={args.recurrence_factor}")
    ...
    eval_model = GPT(
        ...
        recurrence_factor=args.recurrence_factor, pass_scale_init=args.pass_scale_init,
    ).to(device).bfloat16()
